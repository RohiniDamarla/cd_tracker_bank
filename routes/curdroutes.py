...
from app_core.app_config import mail
from flask_mail import Message
from datetime import datetime, date
# ✅ Create Blueprint
curd_bp = Blueprint('curd_bp', __name__)
...
@curd_bp.route('/add', methods=['GET', 'POST'])
@login_required
def add():
    if request.method == 'POST':
        ...
        db.session.add(cd)
        db.session.commit()
        return redirect(url_for('curd_bp.index'))
    return render_template('add_cd.html')

@curd_bp.route('/update/<int:cd_id>', methods=['POST'])
@login_required
def update(cd_id):
...
@curd_bp.route('/delete/<int:cd_id>', methods=['POST'])
@login_required
def delete(cd_id):
    cd = CD.query.filter_by(id=cd_id, user_id=current_user.id).first_or_404()
...
@curd_bp.route('/login', methods=['GET', 'POST'])
def login():
    if request.method == 'POST':
...
@curd_bp.route('/logout')
@login_required
def logout():
    logout_user()
    session.clear()
    flash('You have been logged out.')
    return redirect(url_for('curd_bp.login'))

@curd_bp.route('/register', methods=['GET', 'POST'])
def register():
    if request.method == 'POST':
        ...
        if User.query.filter_by(email=email).first():
            ...
            return redirect(url_for('curd_bp.register'))
...
@curd_bp.route('/forgotpassword', methods=['GET', 'POST'])
def forgotpassword():
    if request.method == 'POST':
        username = request.form['username']
        email = request.form['email']
...
@curd_bp.route('/report_fraud', methods=['POST'])
def report_fraud():
    username = request.form['username']
